...
void GPSService::attachToParser(NMEAParser& _parser) {
    ...
    $GPGSA		- gps receiver operating mode, satellites used in
    position and DOP values $GPGSV		- number of gps satellites in
    view, satellite ID, elevation,azimuth, and SNR $GPRMC		-
    time,date, position,course, and speed data $GPVTG		- course and
    speed information relative to the ground $GPZDA		- 1pps timing
    message $PSRF150	- gps module "ok to send"
    */
    ...
    _parser.setSentenceHandler(
        "GNGSA", [this](const NMEASentence& nmea) { this->read_GNGSA(nmea); });
    _parser.setSentenceHandler(
        "GLGSV", [this](const NMEASentence& nmea) { this->read_GLGSV(nmea); });
    _parser.setSentenceHandler(
        "GNGLL", [this](const NMEASentence& nmea) { this->read_GNGLL(nmea); });
    _parser.setSentenceHandler(
        "GNRMC", [this](const NMEASentence& nmea) { this->read_GNRMC(nmea); });
    _parser.setSentenceHandler(
        "GNVTG", [this](const NMEASentence& nmea) { this->read_GNVTG(nmea); });
    _parser.setSentenceHandler(
        "GNGGA", [this](const NMEASentence& nmea) { this->read_GNGGA(nmea); });
}
    ...
    try {
        ...
        if (this->fix.visibleSatellites < this->fix.trackingSatellites) {
            this->fix.visibleSatellites =
                this->fix.trackingSatellites;  // the visible count is in
                                               // another sentence.
        }
        ...
        NMEAParseError pe("GPS Number Bad Format [$GPGGA] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GPGGA] :: " + ex.message,
                          nmea);
        throw pe;
    }
}
...
void GPSService::read_GNGGA(const NMEASentence& nmea) {
    ...
    [2] 01131.000,E  Longitude 11 deg 31.000' E
    [3] 1            Fix quality (0 = invalid, 1 = GPS fix, 2 = DGPS fix)
    [4] 08           Number of satellites being tracked
    [5] 0.9          Horizontal dilution of position
    [6] 545.4,M      Altitude, Meters, above mean sea level
    [7] 46.9,M       Height of geoid (mean sea level) above WGS84 ellipsoid
    ...
    */
    ...
    try {
        ...
        // Latitude
        this->fix.latitude =
            convertLatLongToDeg(nmea.parameters[1], nmea.parameters[2]);

        // Longitude
        this->fix.longitude =
            convertLatLongToDeg(nmea.parameters[3], nmea.parameters[4]);

        // Fix quality
        this->fix.quality = static_cast<uint8_t>(parseInt(nmea.parameters[5]));

        // Number of satellites
        this->fix.trackingSatellites =
            static_cast<int32_t>(parseInt(nmea.parameters[6]));

        // Horizontal Dilution of Precision (HDOP)
        this->fix.horizontalDilution = parseDouble(nmea.parameters[7]);
        ...
        NMEAParseError pe("GPS Number Bad Format [$GNGGA] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GNGGA] :: " + ex.message,
                          nmea);
        throw pe;
    }
}
...
void GPSService::read_GPGSA(const NMEASentence& nmea) {
    ...
    */
    ...
    try {
        ...
        NMEAParseError pe("GPS Number Bad Format [$GPGSA] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GPGSA] :: " + ex.message,
                          nmea);
        throw pe;
    }
}
...
void GPSService::read_GPGSV(const NMEASentence& nmea) {
    ...
    */
    ...
    try {
        ...
        // can't do this check because the length varies depending on
        // satallites...
        ...
        }  // Also NMEA defaults to 12 visible when chip powers on. Obviously
           // not right.
        ...
        int entriesInPage = (nmea.parameters.size() - 3) >>
                            2;  // first 3 are not satellite info
        ...
        NMEAParseError pe("GPS Number Bad Format [$GPGSV] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GPGSV] :: " + ex.message,
                          nmea);
        throw pe;
    }
}

void GPSService::read_GLGSV(const NMEASentence& nmea) {
    // essentially the same as GPGSV
    ...
    */
    ...
    try {
        ...
        if (status == 'V') {
            ...
            lockupdate = this->fix.setlock(
                false);  // not A or V, so must be wrong... no lock
        }

        this->fix.speed = convertKnotsToKilometersPerHour(parseDouble(
            nmea.parameters[6]));  // received as knots, convert to km/h
        ...
        NMEAParseError pe("GPS Number Bad Format [$GPRMC] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GPRMC] :: " + ex.message,
                          nmea);
        throw pe;
    }
}
...
void GPSService::read_GPVTG(const NMEASentence& nmea) {
    ...
    */
    ...
    try {
        ...
        NMEAParseError pe("GPS Number Bad Format [$GPVTG] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GPVTG] :: " + ex.message,
                          nmea);
        throw pe;
    }
}
...
void GPSService::read_GNVTG(const NMEASentence& nmea) {
    // essentially the same as GPVTG
    this->read_GPVTG(nmea);
}

void GPSService::read_GNGLL(const NMEASentence& nmea) {
    /*  -- EXAMPLE --
    $GNGLL,4916.45,N,12311.12,W,225444,A,*1D

    Where:
    GLL          Geographic Position – Latitude/Longitude
    [0] 4916.45       Latitude 49 deg. 16.45 min. N
    [1] N             N or S (North or South)
    [2] 12311.12      Longitude 123 deg. 11.12 min. W
    [3] W             E or W (East or West)
    [4] 225444        Fix taken at 22:54:44 UTC
    [5] A             Status A=active or V=Void.
    [6] *1D           The checksum data, always begins with *
    */
    ...
    try {
        ...
        // Latitude
        this->fix.latitude =
            convertLatLongToDeg(nmea.parameters[0], nmea.parameters[1]);

        // Longitude
        this->fix.longitude =
            convertLatLongToDeg(nmea.parameters[2], nmea.parameters[3]);

        // Timestamp
        this->fix.timestamp.setTime(parseDouble(nmea.parameters[4]));

        // Status
        char status = nmea.parameters[5].empty() ? 'V' : nmea.parameters[5][0];
        this->fix.status = status;
        bool lockupdate = this->fix.setlock(status == 'A');

        // Notify of lock state change
        if (lockupdate) {
            this->onLockStateChanged(this->fix.haslock);
        }
        this->onUpdate();
    } catch (NumberConversionError& ex) {
        NMEAParseError pe("GPS Number Bad Format [$GNGLL] :: " + ex.message,
                          nmea);
        throw pe;
    } catch (NMEAParseError& ex) {
        NMEAParseError pe("GPS Data Bad Format [$GNGLL] :: " + ex.message,
                          nmea);
        throw pe;
    }
}